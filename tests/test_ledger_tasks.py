...
class DummyApiService:
    ...
    def fetch_inference_traverse(self, *, ledger_id: Optional[str] = None):
        self._maybe_raise()
        return [{"node": "prime", "weight": 0.7}]

    def fetch_inference_memories(self, *, ledger_id: Optional[str] = None):
        self._maybe_raise()
        return [{"text": "hello", "score": 0.92}]

    def fetch_inference_retrieve(self, *, ledger_id: Optional[str] = None):
        self._maybe_raise()
        return {"hits": 3}
